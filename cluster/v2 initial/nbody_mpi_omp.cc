...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        save(t, "time_MPI_THREADED_" + std::to_string(N) + ".txt", "Time");
    ...
    if (rank == 0) {
        std::cout << "Runtime = " << elapsed << " s for N = " << N << " with " << n_ranks << " ranks and " << omp_get_max_threads() << " threads per rank" << std::endl;
    }
