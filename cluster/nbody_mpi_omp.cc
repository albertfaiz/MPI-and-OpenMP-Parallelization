// nbody_mpi_omp.cc - Hybrid MPI+OpenMP Version
#include <mpi.h>
#include <omp.h>
#include <iostream>
#include <fstream>
...
// Arrays for MPI work distribution:
...
std::tuple<Vec, Vec> initial_conditions(const Vec &global_x, const Vec &global_v){
    Vec x_local(ND_end - ND_beg), v_local(ND_end - ND_beg);
    double dx = x_max - x_min, dv = v_max - v_min;
    #pragma omp parallel for
    for (int i = ND_beg; i < ND_end; i++){
        unsigned local_seed = 12345 + i + omp_get_thread_num() + rank*1000;
        std::mt19937 local_gen(local_seed);
        x_local[i - ND_beg] = ran(local_gen) * dx + x_min;
        v_local[i - ND_beg] = ran(local_gen) * dv + v_min;
    }
    Vec x = global_x;
    Vec v = global_v;
    if(x.size() != ND) x.resize(ND, 0.0);
    if(v.size() != ND) v.resize(ND, 0.0);
    for (int i = ND_beg; i < ND_end; i++){
        x[i] = x_local[i - ND_beg];
        v[i] = v_local[i - ND_beg];
    }
    MPI_Allgatherv(MPI_IN_PLACE, ND_end-ND_beg, MPI_DOUBLE, x.data(), countsD.data(), displsD.data(), MPI_DOUBLE, MPI_COMM_WORLD);
    MPI_Allgatherv(MPI_IN_PLACE, ND_end-ND_beg, MPI_DOUBLE, v.data(), countsD.data(), displsD.data(), MPI_DOUBLE, MPI_COMM_WORLD);
    return {x, v};
}
...
int main(int argc, char** argv){
    ...
    for (int i = 0; i <= T; i++)
        t[i] = i * dt;
    Vec m(N, m_0);
    Vec x_global(ND, 0.0), v_global(ND, 0.0);
    Vecs x(T+1), v(T+1);
    std::tie(x[0], v[0]) = initial_conditions(x_global, v_global);
    for (int n = 0; n < T; n++){
    ...
    if(rank == 0){
        save(KE_total, "KE_MPI_OMP_" + std::to_string(N) + ".txt", "Kinetic Energy (MPI+OpenMP)");
